...
import { updateState, setKeyHolderId, hasTwoPlayers } from '../reducer.js'
...
socket.on('remove', function(socketId) {
    if (!Hourglass.game.state.getCurrentState() || Hourglass.game.state.getCurrentState().key !== 'Game') {} else {
...
socket.on('hasTwoPlayers', function(twoPlayersBool) {
 store.dispatch(hasTwoPlayers(twoPlayersBool))
 })
